...
y <- matrix(data = stars$first_award_age,
            ncol = 1)

#create design matrix using data.matrix()
X1 <- stars %>% dplyr::mutate(Int = 1) %>% dplyr::select(Int, breakthrough_age) %>% data.matrix()
```
...
tab <- data.frame(Variable = c("Intercept", "Breakthrough age"),
                  Effect = beta_hat1,
...
tab %<>% mutate(Z = round(Effect/SE,3),
                P = ifelse(pnorm(abs(Z), lower.tail = FALSE) < 0.001, "<0.001", round(pnorm(abs(Z), lower.tail = FALSE),3)))
#print model table
...
my_ls <- function(y, X, cap)
{
  ...
  #create the table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #create a table for the model
  tab <- data.frame(Variable = c("Intercept", "Gender"),
                  Effect = beta_hat,
                  SE = round(diag(sqrt(cov_beta_hat)),3))
  #append Z test statistics and p-values
  tab %<>% dplyr::mutate(Z = round(Effect/SE,3),
                P = ifelse(pnorm(abs(Z), lower.tail = FALSE) < 0.001, 
                           "<0.001", round(pnorm(abs(Z), lower.tail = FALSE),3)))
  #print model table
...
#create design matrix using data.matrix()
X2 <- stars %>% 
  dplyr::mutate(Int = 1, Female = ifelse(gender_traditional == "F", 1, 0)) %>% 
  dplyr::select(Int, Female) %>% data.matrix()
```
...
stars %<>% mutate(gender_traditional = factor(gender_traditional, levels = c("M", "F")))
...
X3 <- stars %>% 
  dplyr::mutate(Int = 1, Female = ifelse(gender_traditional == "F", 1, 0), FemaleBreakthroughAge = Female*breakthrough_age) %>% 
  dplyr::select(Int, breakthrough_age, Female, FemaleBreakthroughAge) %>% data.matrix()
```

Use `my_ls()` again to calculate the same three values for this model, and create a table of the same format. 

```{r, results = 'asis'}
my_ls(y = y, X = X3, cap = "Modeling age at first award on breakthrough age, controlling for gender and the interaction.")
```
...
mod1_plot <- stars %>% ggplot(aes(x = breakthrough_age, y = first_award_age)) + geom_point()
```
...
X_pred <- data.frame(Int = 1, 
                     breakthrough_age = seq(from = 10, to = 55, by = 1)) %>% data.matrix()
...
pred_df <- data.frame(x = seq(from = 10, to = 55, by = 1),
                      y = y_pred)

#add geom_line() layer to mod1_plot with the model predictions for first_award_age
mod1_plot <- mod1_plot + geom_line(data = pred_df, aes(x = x, y = y), col = "blue", lty = 2)
```
...
my_mod_eq <- paste(beta_hat1["Int",1], " + ", beta_hat1["breakthrough_age",1], "x", sep = "")
...
X4 <- stars %>% dplyr::mutate(Int = 1) %>% dplyr::select(Int, breakthrough_age_5yrs) %>% data.matrix()
...
#create design matrix to predict age at first award for breakthrough ages 10-55
X_pred <- data.frame(Int = 1, 
                     breakthrough_age_5yrs = seq(from = 10, to = 55, by = 1)/5) %>%
  data.matrix()
...
#create a dataframe with columns for x and predicted y
pred_df <- data.frame(x = seq(from = 10, to = 55, by = 1)/5,
                      y = y_pred_5yrs)
```
...
#test your paste() statement to make sure you like how it looks
my_mod_eq <- paste(beta_hat1_5yrs["Int",1], " + ", beta_hat1_5yrs["breakthrough_age_5yrs",1], "x", sep = "")

mod1_plot_5yrs <- stars %>% ggplot(aes(x = breakthrough_age_5yrs, y = first_award_age)) + geom_point() + geom_line(data = pred_df, aes(x = x, y = y), col = "red", lty = 3) + annotate(geom = "text", x = 9, y = 72, label = my_mod_eq)
...
X5 <- stars %>% 
  dplyr::mutate(Int = 1, Female = ifelse(gender_traditional == "F", 1, 0)) %>% 
  dplyr::select(Int, breakthrough_age, Female) %>% data.matrix()
```
