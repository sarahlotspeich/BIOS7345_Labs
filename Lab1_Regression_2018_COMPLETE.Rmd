...
noncentral_obs <- replicate(n = num_obs,
                    expr = my_chi_sq(deg_free, mu = nc_mu, sigma = 1))
```
...
tab <- data.frame(Dist = "Chi-Squared", Central = paste(round(mean(central_obs),3), " (", round(var(central_obs),3), ")", sep = ""), NonCentral = paste(round(mean(noncentral_obs),3), " (", round(var(noncentral_obs),3), ")", sep = ""))
```
...
{
  f <- (rchisq(n = 1, df = df_num, ncp = 2*ncp)/df_num)/(rchisq(n = 1, df = df_denom)/df_denom)
  return(f)
}
...
```{r}
central_obs <- replicate(n = num_obs,
                    expr = my_f(df_num, df_denom))
```

Plot the density of the observed F distribution with a range of 0 to 20 on the x-axis.

```{r}
central_plot <- ggplot(data = data.frame(central_obs), aes(x = central_obs)) + geom_density() + xlim(c(0,20)) + ggtitle("F (2,20)")
```
...
```{r}
num <- rchisq(n = 1, df = df_num, ncp = 2*lambda)
denom <- rchisq(n = 1, df = df_denom)
```
...
```{r}
noncentral_obs <- replicate(n = num_obs,
                    expr = my_f(df_num, df_denom, lambda))
```
...
```{r}
#plot the observations
noncentral_plot <- ggplot(data = data.frame(noncentral_obs), aes(x = noncentral_obs)) + geom_density() + xlim(c(0,20)) + ggtitle(TeX('F (2,20,$\\lambda$=2)'))
```
...
```{r}
#compare your density to the true density of a chi-squared(5)
noncentral_plot <- noncentral_plot + stat_function(fun = df, args = list(df1 = df_num, df2 = df_denom, ncp = 2*lambda), col = "red")
```
...
#make a table so we can view these side-by-side
tab <- rbind(tab,
             data.frame(Dist = "F", Central = paste(round(mean(central_obs),3), " (", round(var(central_obs),3), ")", sep = ""), NonCentral = paste(round(mean(noncentral_obs),3), " (", round(var(noncentral_obs),3), ")", sep = "")))
```
...
```{r}
#now write a function
my_t <- function(degrees_freedom,ncp = 0)
{
  t <- rnorm(n = 1, mean = ncp, sd = 1)/sqrt(rchisq(n = 1, df = degrees_freedom)/degrees_freedom)
  return(t)
}
...
```{r}
central_plot <- ggplot(data = data.frame(central_obs), aes(x = central_obs)) + geom_density() + xlim(c(-40,40)) + ggtitle("t(2)")
```
...
central_plot <- central_plot + stat_function(fun = dt, args = list(df = deg_free), col = "red")
```
...
```{r}
#plot the observations
noncentral_plot <- ggplot(data = data.frame(noncentral_obs), aes(x = noncentral_obs)) + geom_density() + xlim(c(-40,40)) + ggtitle(TeX('t (2,$\\lambda$=2)'))
```
...
noncentral_plot <- noncentral_plot + stat_function(fun = dt, args = list(df = deg_free, ncp = mu), col = "red")
```
...
#make a table so we can view these side-by-side
tab <- rbind(tab,
             data.frame(Dist = "t", Central = paste(round(mean(central_obs),3), " (", round(var(central_obs),3), ")", sep = ""), NonCentral = paste(round(mean(noncentral_obs),3), " (", round(var(noncentral_obs),3), ")", sep = "")))
```
...
```{r, results = 'asis'}
xtable(tab, caption ="Comparing the mean/variance of central and non-central distributions.")
```

##Chi-squared distribution

  - Mean and variance of non-central Chi-squared > mean and variance of central Chi-squared

##F distribution

  - Mean and variance of non-central F $>$ mean and variance of central F

##t distribution

  - Mean of the non-central t $<$ mean of central t
  - Variance of non-central t $>$ variance of central t
  - These depend on the value of $\mu$ (via a complicated formula)